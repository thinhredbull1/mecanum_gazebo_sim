...
C = ['red', 'orange', 'yellow', 'blue', "white","white","white","white","white","white","white","white","white"]
...
class ShowSafety():
    def __init__(self):
        ...
        rospy.Subscriber('/scan_safety_cfg_changed_ng', Time, self.cfgChangedCB)
    ...
    def animate(self, i):
        ...
        ax.set_ylim([-2,2])
        if is_running:
            title_obj = plt.title("current job: %i | %s"%(0, "Safety Node is running"))
            plt.setp(title_obj, color='b')
        else:
            title_obj = plt.title("current job: %i | %s"%(0, "Safety Node Stop"))
    ...
    def updateFields(self):
        ...
        self.jobStr = rospy.get_param("/scan_safety_node_ng/job")
        self.footprintStr = rospy.get_param("/scan_safety_node_ng/footprint")
        self.jobJs = json.loads(self.jobStr)
        print(self.jobJs)
        self.footprintJs = json.loads(self.footprintStr)
        print(self.footprintJs)

        self.number_of_fields = len(self.jobJs["safety"])
        ...
        for i in range(number_of_fields):
            ...
            vertexes = self.jobJs["safety"][i]["data"]
        ...
        rb_footprint = self.footprintJs["footprint"]
        rbX = [0]
        rbY = [0]
